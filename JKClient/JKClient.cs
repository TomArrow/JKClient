...
namespace JKClient {
	...
	public sealed partial class JKClient : NetClient {
		...
		private const string UserInfo = "\\name\\" + JKClient.DefaultName + "\\rate\\400000\\snaps\\1000\\model\\kyle/default\\forcepowers\\"+ forcePowersAllDark + "\\color1\\4\\color2\\4\\handicap\\100\\teamtask\\0\\sex\\male\\password\\\\cg_predictItems\\1\\saber1\\single_1\\saber2\\none\\char_color_red\\255\\char_color_green\\255\\char_color_blue\\255\\engine\\jkclient_demoRec\\teamoverlay\\2\\csnec\\1023\\cjp_client\\1.4JAPRO\\csf\\" + jaPlusClientSupportFlags + "\\assets\\0"; // cjp_client: Pretend to be jaPRO for more scoreboard stats
		private const string UserInfoMOH = "\\name\\" + DefaultNameMOH + "\\rate\\400000\\snaps\\1000\\dm_playermodel\\american_ranger\\dm_playergermanmodel\\german_wehrmacht_soldier"; // cjp_client: Pretend to be jaPRO for more scoreboard stats
...
//			this.lastPacketTime = this.realTime;
			...
			if (string.Compare(c, "infoResponse", StringComparison.OrdinalIgnoreCase) == 0)
			{
				...
				if (address == this.serverAddress) {
					...
					} else if (this.Status == ConnectionStatus.Challenging && s.StartsWith("You are banned from this server."))
					{
						...
						this.connectPacketCount = 0;
						this.Status = ConnectionStatus.Connecting;
					} else if (this.Status == ConnectionStatus.Challenging && s.Contains("No or bad challenge for address."))
					{
						this.connectTime = -9999;
						this.infoRequestTime = -9999;
						this.connectPacketCount = 0;
		...
		private void WritePacket() {
			...
			lock (this.netChannel) {
				...
				int serverMessageAckDiffSinceLastPacket = this.serverCommandSequence - this.outPackets[realOldPacketNum].ServerCommandSequence;
				...
				if (this.TrafficReduceUntilClientFps > 0 && realTimeSinceLastPacket > 0 && serverMessageAckDiffSinceLastPacket == 0 && (reliableCount == 0 || reliableSeqDiffSinceLastPacket == 0) && realCount == 0 && realTimeSinceLastPacket < (1000 / this.TrafficReduceUntilClientFps))
				{
					// Reduce network traffic.
					// We have no new commands to sent:
					// - No new usercmds
					// - No new reliable commands that weren't already sent (or all already acknowledged)
					// - No new server message to ack (need this to make delta messages smaller sadly)
				...
				this.outPackets[packetNum].ServerCommandSequence = this.serverCommandSequence;
